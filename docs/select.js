...
SelectHandler.prototype.init = function (value, options) {
  ...
  /////// hh edit     
    ...
    this.nested.matrix(new SVG.Matrix(this.el).translate(0, 0));
  /////// hh edit


    // When deepSelect is enabled and the element is a line/polyline/polygon, draw only points for moving
    if (this.options.deepSelect && ['line', 'polyline', 'polygon','circle'].indexOf(this.el.type) !== -1) {
        this.selectPoints(value);

    } else if (this.el.type ==="circle"){
...
SelectHandler.prototype.drawPoints = function () {
    ...
    for (var i = 0, len = array.length; i < len; ++i) {
        ...
        if (i !== 0 && i !== len - 1) {
          ...
          dragStartEvent = ((k) => () => {
            _this.el.fire('dragStart', { pos: k});
          })(i);
        }

        dragEndEvent = ((k) => () => {
          _this.el.fire('dragEnd', { pos: k});
        })(i);
      ...
      ///////// hh edit                 
                        ...
                        .on('beforedrag', dragStartEvent)
                        .on('dragend', dragEndEvent)

        point._memory.el = this.el
        point._memory.idx = i
      ///////// hh edit                         
...
SelectHandler.prototype.drawCircle = function (cx, cy) {
  ////// hh edit    
    ...
    //                   .center(cx, cy);
    return this.parent.circle(this.options.pointSize)
                      .center(cx, cy);
  ////// hh edit            
};
...
SelectHandler.prototype.updatePointSelection = function () {
    if (!this.pointSelection.set) return
